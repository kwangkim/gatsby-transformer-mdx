...
const {
  toJSX
} = require('@mdx-js/mdx/mdx-hast-to-jsx');
...
const astCacheKey = node => `transformer-mdx-markdown-ast-${node.internal.contentDigest}-${pluginsCacheStr}-${pathPrefixCacheStr}`;

const jsxCacheKey = node => `transformer-mdx-markdown-jsx-${node.internal.contentDigest}-${pluginsCacheStr}-${pathPrefixCacheStr}`;

const mdxHastCacheKey = node => `transformer-mdx-markdown-mdx-hast-${node.internal.contentDigest}-${pluginsCacheStr}-${pathPrefixCacheStr}`;

const headingsCacheKey = node => `transformer-mdx-markdown-headings-${node.internal.contentDigest}-${pluginsCacheStr}-${pathPrefixCacheStr}`;

const tableOfContentsCacheKey = node => `transformer-mdx-markdown-toc-${node.internal.contentDigest}-${pluginsCacheStr}-${pathPrefixCacheStr}`; // ensure only one `/` in new url
...
}, // eslint-disable-next-line no-unused-vars
{
  ...
  return new Promise(async resolve => {
    const mdxAstCompiler = unified().use(toMdAST, {
      commonmark: true,
      footnotes: true,
      pedantic: true,
      ...pluginOptions
    ...
    async function getAST(markdownNode) {
      ...
      const ASTGenerationPromise = new Promise(async resolve => {
        let files = _.values(store.getState().nodes).filter(n => n.internal.type === 'File');
        ...
        files = _.values(store.getState().nodes).filter(n => n.internal.type === 'File');
    ...
    async function getMdxHAST(markdownNode) {
      ...
      const ast = await getAST(markdownNode);
      const hast = mdxAstToMdxHast()(ast); // Save new HTML AST to cache and return
    ...
    async function getJSX(markdownNode) {
      ...
      const hast = await getMdxHAST(markdownNode); // Save new JSX to cache and return
